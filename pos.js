document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTables() {
        ...
        for (let i = 1; i <= tableCount; i++) {
            ...
            if (tableOrders[i] && tableOrders[i].length > 0) {
                ...
                // Optional: Show item count or total
                // const count = tableOrders[i].length;
                // tableBtn.innerHTML += `<br><small>(${count} items)</small>`;
            }
    ...
    function renderProducts(category) {
        ...
        category.items.forEach(item => {
            ...
            `;
            prodEl.onclick = () => addToOrder(item);
            ...
            for (const category of menuData.categories) {
                ...
                if (category.items) {
                    category.items.forEach(item => {
                        ...
                        // Simple padding for alignment (dots)
                        const padding = ".".repeat(Math.max(5, 40 - item.name.length - price.length));
                        content += `${item.name} ${padding} ${price} \n`;
                    });
                }
